...
TRACKER_ROWS = [
    ...
    ["C Basic Syntax and Program Layout", "2", "Semicolons after declarations", "Given a short program containing `int x = 4`, `double y = 2.5`, and a `printf` call without statement endings, fix every missing semicolon and explain why the declarations need them.", ""],
    ["C Basic Syntax and Program Layout", "5", "Tab escape sequence", "Write a `printf` statement that prints two integer values separated by a tab and followed by a newline, using the correct escape sequence for the tab.", ""],
    ["C Basic Syntax and Program Layout", "8", "`return 0` from `main`", "For a small `int main(void)` program that finishes normally, write the final return statement and explain what status it reports to the operating system.", ""],
    ["C Control Flow and Problem Solving", "4", "Factorial loop variable", "Write a factorial loop that counts down from `n` to `1`, then trace `n = 4` and identify which variable must be multiplied into the accumulator each iteration.", ""],
    ["C Control Flow and Problem Solving", "6", "`e` approximation loop bounds", "Write an `approximate_e(int terms)` loop using factorial terms, making sure the division is floating-point and the loop produces exactly `terms` terms.", ""],
    ["C Control Flow and Problem Solving", "9", "Object file compile and link workflow", "For a program split across `main.c` and `maths.c`, write the commands that produce `main.o` and `maths.o`, then link them into an executable.", ""],
    ["C String Processing", "1", "`sizeof` on char arrays", "For `char s[20] = \"Hello world\";`, calculate the storage size and visible character count, then explain why the two numbers are different.", ""],
    ["C String Processing", "2", "Null terminator and `sizeof`", "For `char s[20] = \"Hello world\"; s[0] = '\\0';`, calculate `sizeof(s)` and `strlen(s)`, then identify which result changed.", ""],
    ["C String Processing", "7", "In-place normalise write index", "Trace what goes wrong if an in-place normalise loop increments `write` when punctuation is skipped, then write the corrected loop for `\"A, B!\"` and the required character-function include.", ""],
    ["C String Processing", "10", "Bounded copy loop limit", "For `char dest[5]` copying from `\"Hello\"`, compare loop guards `i <= len - 1` and `i < len - 1`; choose the safe one and show where the null terminator is written.", ""],
    ["C++ Multiple Inheritance", "3", "Protected member access in derived methods", "Given a base class with private `i` and protected `j`, and a derived method containing `i = 0; j = 0;`, mark which assignment compiles and explain the access rule using the derived method context.", ""],
    ["C++ Multiple Inheritance", "4", "Diamond ambiguity is compile-time", "For `Bottom` inheriting from both `Left` and `Right`, where each path contains `Base::value`, decide what happens to `b.value = 10;` and whether the issue is compile-time or runtime.", ""],
    ["C++ Multiple Inheritance", "5", "Selecting one diamond base path", "In a non-virtual diamond with `Bottom b`, write the assignment that sets the `value` reached through `Left` rather than the one reached through `Right`.", ""],
    ["C++ Multiple Inheritance", "7", "Virtual dispatch trace", "Given `Animal::speak` is virtual, `Dog::speak` prints `Bark Fido`, and an `Animal *` points at a `Dog`, trace the call through the base pointer and explain why the dog version runs.", ""],
    ["C++ Multiple Inheritance", "9", "Abstract base class instantiation", "Given `virtual void speak() const = 0;` in `Animal`, decide whether `Animal a(\"Rex\");` compiles, then show what a concrete derived class must provide before it can be instantiated.", ""],
    ["C++ Classes: Access Control and Header Files", "1", "Complete guarded class header", "A header `counter.h` declares a `Counter` class and may be included through several files. Write the complete guarded header around the class declaration so repeated inclusion is safe.", ""],
    ["C++ Classes: Access Control and Header Files", "2", "Scope resolution for member definitions", "In `counter.cpp`, fix `void increment() { count++; }` so it defines the member declared in `Counter`, then explain what the prefix connects the function body to.", ""],
    ["C++ Classes: Access Control and Header Files", "5", "Balanced include guard", "Complete a guarded `sensor.h` header from the opening conditional directive through the final closing directive, using a sensible guard name.", ""],
    ["C++ Classes: Access Control and Header Files", "6", "Repeated include redefinition fix", "`grandparent.h` defines `struct Data` and is included both directly and through `parent.h`. Wrap `grandparent.h` so `child.cpp` does not see two definitions.", ""],
    ["C++ Classes: Access Control and Header Files", "9", "Destructor for owned array", "Given `class Buffer { char *data; public: Buffer(int size) { data = new char[size]; } };`, add the destructor and explain why a default destructor is not enough.", ""],
    ["C++ Classes: Access Control and Header Files", "10", "`new`/`delete` vs `malloc`/`free` for objects", "For a class with a constructor and destructor, compare allocation with `new` and `malloc`, then compare cleanup with `delete` and `free`; say which operations run object lifetime code.", ""],
    ["C Struct Exercises: Points and Rectangles", "1", "Semicolons in struct field declarations", "Repair `typedef struct { double x double y } Point;` so it is a valid C type definition, then declare one `Point` variable.", ""],
    ["C Struct Exercises: Points and Rectangles", "5", "Nested struct field semicolons", "Write a `Rectangle` type containing `Point bottom_left` and `Point top_right`, making sure the field declarations and the whole type definition are correctly terminated.", ""],
    ["C Struct Exercises: Points and Rectangles", "8", "Updating caller-owned struct through pointer", "Fix `void move_point(Point p, double dx, double dy)` so the caller's point is changed, then show the corrected call for a variable named `pt`.", ""],
    ["C Struct Exercises: Points and Rectangles", "9", "Choosing pass-by-value for structs", "A function only reads a small `Point` to calculate a result and must not modify the caller's value. Choose a by-value or pointer parameter and justify the choice.", ""],
    ["C Struct Exercises: Points and Rectangles", "10", "Struct size, padding, and alignment", "Compare `sizeof` for a two-double `Point` with a struct containing `char c; double x;`. Explain why the mixed-field struct may be larger than the simple sum of field sizes.", ""],
    ["Rust Tooling and Cargo", "2", "Cargo as build tool and package manager", "A Rust project uses Cargo where a C project might need separate build and dependency tools. Name the two jobs Cargo handles and give one command or file connected to each job.", ""],
    ["Rust Tooling and Cargo", "3", "`cargo new` project layout", "After running `cargo new hello_rust`, list the generated source entry point path and the main Cargo config files, then say which file contains the starting `main` function.", ""],
    ["Rust Tooling and Cargo", "6", "`cargo run` with unchanged source", "After a successful build, no source files have changed. Explain what `cargo run` does next and whether it recompiles or runs the existing binary.", ""],
    ["Rust Tooling and Cargo", "9", "Opening Rust Book offline", "On a machine with Rust docs installed but no internet connection, write the command that opens the Rust Book and distinguish it from opening the general local Rust documentation.", ""],
    ["Rust Ownership, Closures, Lifetimes, and Smart Pointers", "1", "Closure capture mode for copied values", "Given `let factor = 3; let scale = |n: i32| n * factor;` and then a `move` version of the same closure, explain how `factor` is captured in each case and whether the original `factor` can still be printed.", ""],
    ["Rust Ownership, Closures, Lifetimes, and Smart Pointers", "6", "Returning owned data or input borrows", "Given `fn bad() -> &str { let text = String::from(\"hello\"); &text }`, explain why the returned reference is invalid, then rewrite it once by returning an owned `String` and once by returning a reference to input data.", ""],
    ["Rust Ownership, Closures, Lifetimes, and Smart Pointers", "7", "Dereferencing a borrowed Box", "Complete `fn read_boxed(n: &Box<i32>) -> i32` so it returns the inner integer, then explain what each dereference step is doing.", ""],
    ["Rust Practical Projects and Data", "2", "Unit test module shape", "Inside `src/lib.rs`, write the unit test module for `pub fn square(n: i32) -> i32 { n * n }`, importing the parent module correctly and ending the assertion statement properly.", ""],
    ["Rust Practical Projects and Data", "3", "Integration tests use public API", "Given a private `helper` used by public `public_api`, explain why `tests/basic.rs` cannot import `helper`, then write the valid import and assertion against the public function.", ""],
    ["Rust Practical Projects and Data", "4", "Command-line argument positions", "For `cargo run -- 12 2.5`, label `args[0]`, `args[1]`, and `args[2]`, including what kind of value `args[0]` usually contains.", ""],
    ["Rust Practical Projects and Data", "5", "Parse error handling with `match`", "Replace `let count: i32 = args[1].parse().unwrap();` with a `match` that uses the parsed value on success and prints an error before returning on invalid input.", ""],
    ["Rust Practical Projects and Data", "6", "Parse target type syntax", "Complete `let n = text.trim().____?;` inside `parse_count` so the parse target is `i32` and the surrounding `Result` can return parse errors early.", ""],
    ["Rust Practical Projects and Data", "8", "Accumulating totals in a `HashMap`", "A totals map already contains `tea -> 2.50`. Given another tea item with quantity 3 and unit price 1.25, write the update so the stored tea total becomes 6.25 rather than being replaced or ignored.", ""],
    ["Rust Practical Projects and Data", "9", "Dereferencing iterator values", "In `for (index, value) in values.iter().enumerate()`, complete the comparison that returns `Some(index)` when the borrowed value is greater than `threshold`.", ""],
    ["Rust Practical Projects and Data", "10", "Tuple destructuring from borrowed iteration", "Given `for (name, score) in rows.iter()`, explain what `(name, score)` pulls out and why those bindings are references rather than owned values.", ""],
    ["C++ Build Systems, STL Algorithms, and Shared Ownership", "3", "CMake role vs compiler role", "Given `cmake -S . -B build` followed by `cmake --build build`, explain what CMake configures or drives, and whether the actual compile/link work is still done by compiler tools such as `g++`.", ""],
    ["C++ Build Systems, STL Algorithms, and Shared Ownership", "4", "`binary_search` requires matching sorted order", "Given `std::vector<int> values = {8, 2, 5, 1};` followed by `std::binary_search(values.begin(), values.end(), 5)`, add the minimum fix for the default search and explain why a descending sort comparator would not match that default search.", ""],
    ["C++ Build Systems, STL Algorithms, and Shared Ownership", "5", "Descending `std::sort` comparator direction", "Complete a `std::sort` comparator for largest-first integer scores, then explain what the comparator's true result means about the order of its two arguments.", ""],
    ["C Builds, Libraries, and Buffering", "1", "Flushing buffered stdout", "Given `printf(\"A\"); printf(\"B\"); fflush(stdout); printf(\"C\\n\");`, explain what `fflush(stdout)` forces to happen before the newline is printed.", ""],
    ["C Builds, Libraries, and Buffering", "6", "Makefile object compile rule", "Complete the Makefile command for `stack.o: stack.c stack.h`, using the compiler, C standard flag, compile-only step, source file, and named object output.", ""],
    ["C Builds, Libraries, and Buffering", "5", "Recompile one source and reuse object file", "Given `stack.o` already exists, `main.c` changed, and `stack.c` did not change, write the commands that recompile only `main.c` into `main.o` and then link with the existing `stack.o`.", ""],
    ["C Builds, Libraries, and Buffering", "7", "Recompile objects then relink executable", "Given `program` depends on `main.o` and `stack.o`, and both object files depend on `stack.h`, explain exactly what happens to the two object files and to `program` after `stack.h` changes.", ""],
    ["C Builds, Libraries, and Buffering", "9", "Make target already up to date", "Given `make: 'program' is up to date`, explain what this says about whether `program` exists and how its timestamp compares with the files it depends on.", ""],
    ["C Builds, Libraries, and Buffering", "10", "Makefile clean command", "Write a `clean` recipe command that removes all `.o` files and the `program` executable, then explain what `-f` and `*.o` do.", ""],
    ["Rust Control Flow, Sorting, and Tooling", "3", "Semicolon turns return expression into statement", "Given `fn double(n: i32) -> i32 { n * 2; }`, explain what the function body evaluates to and fix it so it returns an `i32`.", ""],
    ["Rust Control Flow, Sorting, and Tooling", "4", "`if` as an expression needs both branch values", "Given `let score = 72;`, write one `let result = ...;` binding using `if` as an expression to produce `\"pass\"` or `\"fail\"`, and explain why the assigned expression needs an `else` branch.", ""],
    ["Rust Control Flow, Sorting, and Tooling", "8", "Rust `sort_by` with descending primary key", "Given `let mut items = vec![(2, \"red\"), (5, \"blue\"), (5, \"amber\")];`, write the `sort_by` call with `|a, b|` that sorts by count descending and then word ascending.", ""],
    ["Rust Control Flow, Sorting, and Tooling", "9", "Sorting floats with `partial_cmp`", "Given `let mut readings = vec![(0.5, 2), (0.1, 9), (0.5, 1)];`, write the `sort_by` call with `|a, b|` that sorts by float ascending and then id ascending.", ""],
    ["Rust Control Flow, Sorting, and Tooling", "10", "Sort then collect top entries", "Given a mutable vector of `(String, i32)` scores, sort highest score first and collect an owned `Vec` containing the top two entries.", ""],
    ["ECM2433 Mock Paper 1", "1b", "Makefile clean rule", "For the mock project with `main.c`, `stats.c`, and `stats.h`, write the full Makefile fragment for `report`, then include the `clean` recipe that removes all object files and the `report` executable.", ""],
    ["ECM2433 Mock Paper 1", "1c", "Heap string copy allocation", "In the `copy_label(const char *src)` stub, fill in `char *copy = ...`, the allocation failure check, and the copy step so the result includes room for the string terminator.", ""],
    ["ECM2433 Mock Paper 1", "1d", "`qsort` comparator ordering", "For `typedef struct { char name[16]; int score; } Entry;`, complete `compare_entries` and `sort_entries` so `qsort` orders score descending and name ascending on ties.", ""],
    ["ECM2433 Mock Paper 1", "1e", "Dangling pointer after `free`", "For the mock code `int *p = malloc(sizeof *p); *p = 10; free(p); printf(\"%d\\n\", *p);`, explain why `p` is dangling and what C behaviour dereferencing it gives.", ""],
    ["ECM2433 Mock Paper 1", "2b", "Rule of three for raw heap arrays", "For the mock `Buffer` class with `char *data`, `new char[size]`, and `~Buffer() = default`, identify the leak and explain why a raw-owning fix also needs a copy constructor and copy assignment operator.", ""],
    ["ECM2433 Mock Paper 1", "2d", "`shared_ptr` last owner lifetime", "Trace the mock `shared_ptr`/`weak_ptr` code with `a`, `watch`, `b = a`, and `a.reset()`: decide whether `watch.lock()` succeeds and when the shared int is destroyed.", ""],
    ["ECM2433 Mock Paper 1", "2e", "Catching exceptions by const reference", "For `catch (const std::invalid_argument& error)` after throwing `std::invalid_argument(\"negative age\")`, explain why const reference is used and what `error.what()` prints.", ""],
    ["ECM2433 Mock Paper 1", "3d", "Float sorting unwrap after `partial_cmp`", "Given `let mut readings = vec![(0.5, 2), (0.1, 9), (0.5, 1)];`, write the `sort_by` call for float ascending and id descending, including `partial_cmp(...).unwrap()`.", ""],
    ["ECM2433 Mock Paper 1", "3e", "Semicolon after Rust `let` parse statement", "In the mock `parse_count` function returning `Result<i32, ParseIntError>`, write the parsing `let n = ...?;` line and explain why that line needs a semicolon but `Ok(n)` does not.", ""],
    ["ECM2433 Mock Paper 1", "4a", "Channel send and dropping original sender", "In the mock `mpsc` program with `let (tx, rx) = mpsc::channel()` and `for id in 0..4`, spawn workers that send `id * 10`, then explain why `drop(tx)` is needed before `for value in rx` can finish.", ""],
    ["ECM2433 Mock Paper 1", "4b", "Mutex lock result and guard lifetime", "In the mock `Arc<Mutex<i32>>` counter loop, complete the spawned closure so it locks, unwraps, increments through the guard, and explain when the guard unlocks the mutex.", ""],
    ["ECM2433 Mock Paper 1", "4c", "Generic bounds for compare and display", "For the mock `show_larger<T>` body using `if a > b` and `println!(\"{winner}\")`, fix the generic signature and name the trait bounds needed for those two operations.", ""],
    ["ECM2433 Mock Paper 1", "4d", "Boxed recursive enum size", "For the mock enum `List { Cons(i32, List), Nil }`, replace the recursive field with `Box<List>` and explain why the boxed pointer gives the enum a known size.", ""],
]
