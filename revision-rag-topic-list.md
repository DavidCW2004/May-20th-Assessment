...
| R | Types and conversions: integer division, signed vs unsigned behaviour, promotion, casting, chars, floats, booleans, `typedef`, `size_t`, structs as data types, enums/unions. | No |
...
| G | Custom string routines: `mystrlen`, `string_copy`, pointer-based string functions. | No |
...
| R | RAII vs manual `malloc`/`free`. | No |
| R | Smart-pointer ownership models in C++ vs Rust ownership/reference-counting. | No |
...
| R | Array/slice processing, threshold-style scans, and tuple destructuring. | No |
...
| A | Unit tests, integration tests, assertion patterns, and float comparisons with tolerances. | No |
...
| A | Smart pointers beyond the lecture slides: `Box<T>` and `Rc<T>`. | No |
